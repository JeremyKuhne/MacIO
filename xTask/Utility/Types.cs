...
{
    /// <summary>
    ///  Converts the source object to the desired type if possible.  Returns default of T if it cannot.
    ///  Nullable tolerant.
    /// </summary>
    /// <remarks>
    ///  This method tries to be as flexible as possible, allowing conversion to nullable for source primitives
    ///  (including enums). This allows you to ask for nullable of int for an underlying int source object- this way
    ///  you can know the conversion failed if you care to.  It also also allows you to ask for the primitive type
    ///  for a source nullable type if you don't care about conversion success.
    /// </remarks>
    public static T? ConvertType<T>(object? source)
    {
        ...
        if (type.IsPrimitive)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    // One special case here- conceptually converting a number to a bool makes a lot of sense as
                    // "0" == false for *all* primitive types, but converting a string or char representation of a
                    // number will fail (only "true" and "false" are valid). If we have a source object that can be
                    // converted to double, use that for the conversion (could potentially use decimal, but
    ...
    {
        Debug.WriteLine($"Unable to convert value '{source ?? "<null>"}'.  Exception follows: \n{e}");
    }
}
