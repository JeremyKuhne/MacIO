...
public static class Strings
{
    ...
    public static string FindRightmostCommonString(params string[] values)
    {
        ...
        foreach (string value in values)
        {
            ...
            int i = 1;
            while (i <= rightmost.Length && i <= value.Length && rightmost[^i] == value[^i])
            {
                i++;
            }
            ...
            {
                rightmost = rightmost[(rightmost.Length - i + 1)..];
            }
        }

        return rightmost ?? string.Empty;
    }
    ...
    {
        return string.IsNullOrWhiteSpace(value) ? Resources.NoValue : value;
    }
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown if <paramref name="length"/> is less than zero.</exception>
        ...
        StringComparison comparisonType = StringComparison.Ordinal)
    {
        ...
        if (firstIndex < 0 || first.Length - firstIndex < 0)
            throw new ArgumentOutOfRangeException(nameof(firstIndex));
        if (secondIndex < 0 || second.Length - secondIndex < 0)
